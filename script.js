...
// const SYSTEM_PROMPT =
//   "You are the Interviewer. Respond succinctly and professionally. Avoid markdown tables.";
...
//   const tag = document.createElement("div"); tag.className = "tag"; tag.textContent = `${role}:`;
//   const bubble = document.createElement("div"); bubble.className = "bubble"; bubble.innerHTML = html;
//   row.append(tag, bubble); chat.append(row); chat.scrollTop = chat.scrollHeight;
//   return bubble;
// }
...
// // --- LLM session (fixed params) ---
// async function ensureSession() {
//   if (!("LanguageModel" in self)) {
//     showError("Prompt API not available. Use Chrome (Dev/Canary) with AI flags and HTTPS.");
//     throw new Error("No Prompt API");
//   }
//   if (!session) {
...
// const SpeechRecognition = self.SpeechRecognition || self.webkitSpeechRecognition || null;

// function initRecognition() {
//   if (!SpeechRecognition) {
//     startBtn.disabled = true; stopBtn.disabled = true;
//     showError("Speech Recognition API not available in this browser.");
//     return;
//   }
...
//     // Fired both on user stop and end-of-speech
//     listening = false;
...
//       const bubble = addMessage("Interviewer", "…");
//       let result = "", prev = "";
//       const stream = await sess.promptStreaming(text);
...
//         prev = chunk;
//       }
//       speak(stripMarkdown(result));
//     } catch (err) {
//       addMessage("Interviewer", `<i>Error:</i> ${DOMPurify.sanitize(err.message)}`);
...
//   } catch (e) {
//     showError("Failed to start mic. Ensure HTTPS and grant microphone permission.");
//   }
// });
...
// // Greet
// addMessage("Interviewer", "Hello Candidate — click “Start listening”, speak your question, then “Stop listening” or pause to finish.");


/**
 * Voice Interview — Candidate ↔ Interviewer
 * Interviewer starts with "Introduce yourself", then asks follow-ups.
 * Passes job description + previous question + user response on each turn.
...
// // ───────────────────────────────────────────────────────────────────────────────
// // 1) Configure your JOB DESCRIPTION here
// // ───────────────────────────────────────────────────────────────────────────────
// const JOB_TITLE = "AI Scientist";
// const JOB_DESCRIPTION = `
// We are hiring an AI Scientist to research and build state-of-the-art models across
// foundation models, computer vision, and multimodal learning. Responsibilities include:
// experiment design, model training/tuning (PyTorch/JAX), evaluation, and deploying
// prototypes. Strong publication track record and experience with LLMs, diffusion, or
// self-supervised learning preferred. Collaborate cross-functionally with product and
// platform teams to ship reliable AI features.
// `.trim();
...
// // ───────────────────────────────────────────────────────────────────────────────
// const SYSTEM_PROMPT =
//   `You are the Interviewer. You are conducting a structured job interview for the role "${JOB_TITLE}".
// Ask one concise question at a time. Keep a professional tone. Do NOT answer your own questions.
// Prefer specific, behavior-based, or technical questions. Avoid long preambles and avoid markdown tables.`;

// const STARTER_PROMPT = `
// Hi Gemini, You're an AI Interviewer and you are going to recruit a candidate for the following job role: ${JOB_TITLE}.
// You're taking a job interview. Use this job description:

// ${JOB_DESCRIPTION}

// Start by asking exactly this first question only: "Introduce yourself".
// After that, you will continue the interview one question at a time based on the candidate's answers.
// Do not add analysis or commentary—just the interviewer's question text.
// `.trim();

// const NEXT_QUESTION_PROMPT = ({ prevQuestion, userAnswer }) => `
// You're taking a job interview for "${JOB_TITLE}".
...
// Now ask the next best single question based strictly on the candidate's response and the job description.
// Ask exactly one question. No preamble, no meta-commentary, no hints, and do not answer it yourself.
...
// // ───────────────────────────────────────────────────────────────────────────────
// // 5) Model session (fixed params as requested)
// // ───────────────────────────────────────────────────────────────────────────────
// let session = null;
// async function ensureSession() {
//   if (!("LanguageModel" in self)) {
//     showError("Prompt API not available. Use Chrome (Dev/Canary) with the AI flags over HTTPS.");
//     throw new Error("No Prompt API");
//   }
//   if (!session) {
//     session = await LanguageModel.create({
...
// // ───────────────────────────────────────────────────────────────────────────────
// let previousQuestion = "";  // interviewer’s last question
// let listening = false;

// // ───────────────────────────────────────────────────────────────────────────────
// // 7) Start the interview immediately (first question)
// // ───────────────────────────────────────────────────────────────────────────────
...
//     let result = "", prev = "";
//     const stream = await sess.promptStreaming(STARTER_PROMPT);
//     for await (const chunk of stream) {
//       const diff = chunk.startsWith(prev) ? chunk.slice(prev.length) : chunk;
//       result += diff;
...
//       prev = chunk;
//     }
//     previousQuestion = stripMarkdown(result).trim();
//     speak(previousQuestion);
//   } catch (err) {
//     addMessage("Interviewer", `<i>Error:</i> ${DOMPurify.sanitize(err.message)}`);
//     showError(err.message || "Unknown error");
//   }
...
//     // Candidate bubble
//     addMessage("Candidate", DOMPurify.sanitize(userAnswer));
//     transcriptEl.textContent = "";

//     // Compose the next-question prompt with job description + prev Q + user response
//     const prompt = NEXT_QUESTION_PROMPT({
//       prevQuestion: previousQuestion || "(none yet)",
//       userAnswer,
//     });

//     // Ask LLM for the next interviewer question and speak it
//     try {
//       const sess = await ensureSession();
//       const bubble = addMessage("Interviewer", "…");
//       let result = "", prev = "";
...
//       previousQuestion = plain;   // store for next turn
//       speak(plain);
//     } catch (err) {
//       addMessage("Interviewer", `<i>Error:</i> ${DOMPurify.sanitize(err.message)}`);
//       showError(err.message || "Unknown error");
...
// stopBtn.addEventListener("click", () => {
//   if (recognition && listening) recognition.stop(); // triggers onend
// });
...
//   // Restart interview with the first question again
//   (async () => {
//     try {
//       const sess = await ensureSession();
//       const bubble = addMessage("Interviewer", "…");
...
/**
 * Voice Interview — Candidate ↔ Interviewer (no user response context)
 * The interviewer runs a structured interview without referencing the user's past answers.
 * (c) 2024 Google — Apache-2.0
 */
...
//     // Get next question (NO user response passed)
//     try {
